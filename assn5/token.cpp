...
void pretty_print(int h){
	...
	if (strcmp(buffer,"if")==0||strcmp(buffer,"else")==0||strcmp(buffer,"for")==0||strcmp(buffer,"return")==0){
	printf("%4d  |\t Reserved Keyword     | %20s | %3d %3d  |\n",++token_id,buffer,location[0],y);
	//fprintf(fw,"%4d  |\t Reserved Keyword     | %20s | %3d %3d  |\n",++token_id,buffer,location[0],y);

	}
	if(strcmp(buffer,"int")==0||strcmp(buffer,"float")==0||strcmp(buffer,"void")==0){
	printf("%4d  |\t DataType             | %20s | %3d %3d  |\n",++token_id,buffer,location[0],y);
	}else if(numFlag==1){
	printf("%4d  |\tNumerical Expres      | %20s | %3d %3d  |\n",++token_id,buffer,location[0],y);
	fprintf(fw,"numbers ");
	}else if(strcmp(buffer,"for")!=0){printf("%4d  |\t   Identifiers        | %20s | %3d %3d  |\n",++token_id,buffer,location[0],y);
	fprintf(fw,"identifiers ");
	...
	/* For rep form conversion */
	if (strcmp(buffer,"if")==0) fprintf(fw,"if ");
	if (strcmp(buffer,"main")==0) fprintf(fw,"main ");
	if (strcmp(buffer,"int")==0) fprintf(fw,"int ");
	if (strcmp(buffer,"float")==0) fprintf(fw,"float ");
	if (strcmp(buffer,"void")==0) fprintf(fw,"void ");
	if (strcmp(buffer,"for")==0) fprintf(fw,"for ");
	freeBuffer();
